...
describe('SimpleAccountAPI', () => {
  ...
  before('init', async () => {
    ...
    const accountFactory = await new SimpleAccountFactory__factory(signer)
    ...
    it('should parse FailedOp error', async () => {
      const err = await entryPoint.handleOps([userOp], beneficiary)
        .catch(error => {
          ...
          const str = ethers.utils.toUtf8String('0x' + errorData.substring(202))
          return str.trim().replace(/\u0000/g, '')
        })
      console.log(err)
      await expect(err).to.equal('AA23 reverted: ECDSA: invalid signature length'.trim())
    })
